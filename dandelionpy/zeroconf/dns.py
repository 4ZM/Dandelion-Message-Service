...
import traceback
import logging
from functools import reduce    #FIX: reduce is removed from global namespace in python 3
...
class DNSIncoming(object):
    ...
    def readCharacterString(self):
        ...
        print("read char", self.data[self.offset])
    ...
    def readName(self):
        ...
        while 1:
            # FIX: assuming self.data[off] is an int
            len = self.data[off]
            #print("dns, len:", len)
            """
            print("ord, dns", self.data[off])
            if self.data[off].isdigit():
                len = self.data[off]
            else:
        ...
        if isinstance(properties, dict):
            ...
            for key in properties:
                ...
                if value is None:
                    ...
                    suffix = ''.encode('utf-8')
                #print("key", key, "suffix", suffix)
                # FIX: list.append('='.join((key, str(suffix) )))
                suffix = str(suffix)
            ...
            for item in list:
                ...
                if len(result) == 0:
                    result = ' '
                c = chr(len(item))
                #ORIGINAL :result = ''.join( (result, struct.pack('!c', '1'.encode(encoding='utf_8', errors='strict') ), item))
                s = struct.pack('!c', c.encode(encoding='utf_8', errors='strict') )
                try:
                    
                    result = ''.join( (result,  str(s) , item) )
